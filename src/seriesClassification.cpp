...
bool seriesClassificationTemplate<T>::train(const std::vector<trainingSeriesTemplate<T> > &seriesSet)
{
  ...
  else
  {
    ...
    {
      //Global
      const size_t newLength { trainingSeries.input.size() };
      if (newLength < minLength) minLength = newLength;
      if (newLength > maxLength) maxLength = newLength;
      
      //Per Label
      typename std::map<std::string, minMax<std::size_t> >::iterator it { lengthsPerLabel.find(trainingSeries.label) };
      if (it != lengthsPerLabel.end())
      {
        std::size_t newLength { trainingSeries.input.size() };
        if (newLength < it->second.min) it->second.min = static_cast<int>(newLength);
        if (newLength > it->second.max) it->second.max = static_cast<int>(newLength);
      }
      else
      {
        minMax<std::size_t> tempLengths;
...
T seriesClassificationTemplate<T>::runParallel(const std::vector<std::vector<T>> &inputSeries, std::string label)
{
  ...
  else
  {
    ...
    for (const auto series : allTrainingSeries)
    {
    ...
    for (std::size_t i {}; i < runningThreads.size(); ++i)
    {
...
std::string seriesClassificationTemplate<T>::runContinuous(const std::vector<T> &inputVector)
{
  ...
  {
    if (isTraining) throw std::runtime_error("can't run a model during training");
...
{
  std::size_t labelMinLength {};
  typename std::map<std::string, minMax<std::size_t> >::const_iterator it { lengthsPerLabel.find(label)};
...
typename seriesClassificationTemplate<T>::template minMax<T> seriesClassificationTemplate<T>::calculateCosts(std::string label) const
{
  ...
  for (size_t i {}; i < (allTrainingSeries.size() - 1); ++i) //these loops are a little different than the two-label case
  {
    if (allTrainingSeries[i].label == label)
    {
      foundSeries = true;
